...
void ObjectShow(TObject obj, HDC dc)
{
    ...
    if (obj.oType == 'p')
        Rectangle(dc, (int)(obj.pos.x - cam.x), (int)(obj.pos.y - cam.y),
            (int)(obj.pos.x + obj.size.x - cam.x), (int)(obj.pos.y + obj.size.y - cam.y));
    else
        Ellipse(dc, (int)(obj.pos.x - cam.x), (int)(obj.pos.y - cam.y),
            (int)(obj.pos.x + obj.size.x - cam.x), (int)(obj.pos.y + obj.size.y - cam.y));

    //BOOL(*shape)(HDC, int, int, int, int);
    //shape = obj.oType == 'e' ? (BOOL(*)(HDC, int, int, int, int))Ellipse : (BOOL(*)(HDC, int, int, int, int))Rectangle;
...
int main()
{
    ...
    wcl.hCursor = LoadCursorA(NULL, (LPCSTR)IDC_CROSS);
    RegisterClassA(&wcl);

    HWND hwnd;
    hwnd = CreateWindowA("my Window", "WinAPI shooter", WS_SYSMENU, 10, 10, 640, 480, NULL, NULL, NULL, NULL);
